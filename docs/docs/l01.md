...
# SANS SEC540 Cloud Security Simulator Setup Guide (L01)
...
- Provided by you:
    ...
    - OpenAPI compatible API key (if needed)
...
- Download the [sec540-l01.tf](./assets/sec540-l01.tf) launch template (e.g. *sec540-l01-ami.tf*) into the `~/sec540` directory.
...
- The `terraform apply` command will take a few minutes to complete. Once it is finished, you will see the following:
    ...
    environment_summary = <<EOT
      Latest AMI:  ami-06cb88e1e8c394247 - sans-sec540-l01.26.1.0
...
- Use the `labs-start-section-1-aws` command found in the **Lab 1.0 - AWS Cloud Account Setup** instructions to populate the `~/config/aws-config.json` file with the `devsecops` access key id, secret access key, and region. Then, complete the remaining set up steps.
...
- Follow the [Set up authentication option 3](https://learn.microsoft.com/en-us/entra/identity-platform/howto-create-service-principal-portal#option-3-create-a-new-client-secret){target="_blank"} documentation to create a new client secret for the `devsecops` Azure Service Principal.
...
- Use the `labs-start-section-1-az` command found in the **Lab 1.0 - Azure Cloud Account Setup** instructions to populate the `~/config/azure-config.json` file with the `devsecops` client id, client secret, tenant id, and subscription id. Then, complete the remaining set up steps.

## Lab Service Access

Accessing the labs through the SOCKS5 proxy (sans.labs) instead of the Traefik reverse proxy (sec540.cloud) changes the domain name used to access the lab services. The following table shows the mapping of the lab service domain names when accessed through the SOCKS5 proxy:

- Code: `https://code.sans.labs` (instead of `code.unique-id.sec540.cloud`)
- GitLab: `https://gitlab.sans.labs` (instead of `gitlab.unique-id.sec540.cloud`)
- Vault: `https://vault.sans.labs` (instead of `vault.unique-id.sec540.cloud`)
- Moonlight Monitoring: `https://moonlight.sans.labs` (instead of `moonlight.unique-id.sec540.cloud`)
- Defect Dojo: `https://dojo.sans.labs` (instead of `dojo.unique-id.sec540.cloud`)
- Keycloak: `https://login.sans.labs` (instead of `login.unique-id.sec540.cloud`)
- DM Web Application: `https://aws.dm.paper` or `https://az.dm.paper` (instead of `dm-[aws|az].unique-id.sec540.cloud`)
- DM API Gateway: `https://gateway-aws.dm.paper` or `https://gateway-az.dm.paper` (instead of `dm-gateway-[aws|az].unique-id.sec540.cloud`)
